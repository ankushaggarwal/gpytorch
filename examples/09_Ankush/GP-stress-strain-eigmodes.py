...
u,v = torch.symeig(observed_pred.covariance_matrix,eigenvectors=True)
imp_modes = u/torch.norm(u)>0.05
imp_eigvals = u[imp_modes]
imp_vecs = v[:,imp_modes]
n_modes = torch.sum(imp_modes)
# Initialize plot
f, ax = plt.subplots(1, 1, figsize=(10, 6))
for i in range(n_modes):
    # Plot training data as black stars
    ax.plot(test_x.detach().numpy(), imp_vecs[:,i].detach().numpy())
ax.set_ylabel('Stress eigenmodes')
