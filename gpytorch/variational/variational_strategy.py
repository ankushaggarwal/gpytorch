...
class VariationalStrategy2(_VariationalStrategy):
    ...
    def __init__(self, model, inducing_points, inducing_index, variational_distribution, learn_inducing_locations=True):
        super().__init__(model, inducing_points, variational_distribution, learn_inducing_locations)
    ...
    def forward(self, x, inducing_points, inducing_values, variational_inducing_covar=None, x_index=None, **kwargs):
        ...
        # Covariance terms
        num_induc = torch.sum(self.inducing_index).item() #inducing_points.size(-2)
        test_mean = full_output.mean[..., num_induc:]
        induc_induc_covar = full_covar[..., :num_induc, :num_induc].add_jitter()
        induc_data_covar = full_covar[..., :num_induc, num_induc:].evaluate()
        data_data_covar = full_covar[..., num_induc:, num_induc:]
        #print(induc_induc_covar.shape,induc_data_covar.shape,data_data_covar.shape)
        #print(inducing_values,test_mean.shape)
        ...
        if L.shape != induc_induc_covar.shape:
            ...
            L = self._cholesky_factor(induc_induc_covar)
        interp_term = L.inv_matmul(induc_data_covar.double()).to(full_inputs.dtype)
        #print(L.shape,interp_term.transpose(-1,-2).shape)
    ...
    def __call__(self, x, prior=False, **kwargs):
        ...
        #print("__call__ called",x.shape,prior)
        stack = inspect.stack()
        the_class = stack[1][0].f_locals["self"].__class__.__name__
        the_method = stack[1][0].f_code.co_name
        #print("I was called by {}.{}()".format(the_class, the_method))
